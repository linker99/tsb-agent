...
TEST(DomainTest, DomainDestroyEdgeCases)
{
    ...
    VirtrustRc rc = DomainDestroy(conn, "test-domain-1", DomainDestroyFlags::DOMAIN_DESTROY_NONE, false);
    EXPECT_EQ(VirtrustRc::ERROR, rc);

    // Test shut up VM with isOnlyTsb=true and UUID
    rc = DomainDestroy(conn, "12345678-1234-1234-1234-123456789002", DomainDestroyFlags::DOMAIN_DESTROY_NONE, true);
    EXPECT_EQ(VirtrustRc::ERROR, rc);

    // Test running VM with isOnlyTsb=true and running domain UUID
    rc = DomainDestroy(conn, "12345678-1234-1234-1234-123456789003", DomainDestroyFlags::DOMAIN_DESTROY_NONE, true);
    EXPECT_EQ(VirtrustRc::OK, rc);

    // Test with another-shut-off-domain name when isOnlyTsb=false
    rc = DomainDestroy(conn, "another-shut-off-domain", DomainDestroyFlags::DOMAIN_DESTROY_NONE, false);
    EXPECT_EQ(VirtrustRc::ERROR, rc);

    // Test with isOnlyTsb=true using UUID from another-shut-off-domain
    rc = DomainDestroy(conn, "12345678-1234-1234-1234-123456789004", DomainDestroyFlags::DOMAIN_DESTROY_NONE, true);
    EXPECT_EQ(VirtrustRc::ERROR, rc);
...
TEST(DomainTest, DomainStartEdgeCases)
{
    ...
    VirtrustRc rc = DomainStart(conn, "12345678-1234-1234-1234-123456789001", DOMAIN_START_NONE, false);
    EXPECT_EQ(VirtrustRc::ERROR, rc);

    // Test with isOnlyTsb=true and UUID (flags should be ignored)
    rc = DomainStart(conn, "12345678-1234-1234-1234-123456789002", DOMAIN_START_NONE, true);
    EXPECT_EQ(VirtrustRc::OK, rc);
...
TEST(DomainTest, DomainList)
{
    ...
    domainInfos.clear();
    rc = DomainList(conn, DomainListFlags::LIST_DOMAINS_ACTIVE | DomainListFlags::LIST_DOMAINS_INACTIVE, domainInfos,
                    false);
    EXPECT_EQ(VirtrustRc::OK, rc);
}
...
TEST(DomainTest, DomainListDetailed)
{
    ...
    if (!domainInfos.empty()) {
        for (const auto &[name, info] : domainInfos) {
            ...
            (void)testMemory;
            (void)testNrVirtCpu;
            (void)testCpuTime; // Suppress unused warnings

            // Check if domain name matches our mock domains
            bool isMockDomain = (name == "test-domain-1" || name == "test-domain-2" || name == "running-domain" ||
                                 name == "another-shut-off-domain");
            if (isMockDomain) {
...
TEST(DomainTest, ConnCtxEdgeCases)
{
    ...
    std::vector<std::string> testUris = {"qemu+tls://host/system", "qemu+ssh://user@host/system",
                                         "qemu+tcp://host:16509/system", "xen:///", "test:///default"};
