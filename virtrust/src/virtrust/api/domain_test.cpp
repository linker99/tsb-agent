...
// Integration Tests (if Mock is available)
#ifdef VIRTRUST_MOCK
TEST(DomainTest, MockIntegration)
{
    // Tests that work with mock implementations
    auto &conn = GetGlobalConn();
    ASSERT_NE(conn, nullptr) << "Failed to create global connection";

    // These should work consistently with mocks
    std::unordered_map<std::string, DomainInfo> domainInfos;
    VirtrustRc rc = DomainList(conn, DomainListFlags::LIST_DOMAINS_ACTIVE, domainInfos, false);
    EXPECT_EQ(rc, VirtrustRc::OK);

    // Verify we get some mock data
    EXPECT_EQ(false, domainInfos.empty());
}
#endif
