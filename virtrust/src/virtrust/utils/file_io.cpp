...
{
#ifndef __arm64__
    in_.exceptions(std::ifstream::badbit | std::ios::failbit);
#endif
    try {
    ...
    : fileName_(std::move(fileName)), exitFailInDestructor_(exitFailInDestructor)
{
    ...
    out_.exceptions(std::ifstream::badbit | std::ifstream::failbit);
    try {
        out_.open(fileName_, std::ofstream::binary | (trunc ? std::ofstream::trunc : std::ofstream::app));
